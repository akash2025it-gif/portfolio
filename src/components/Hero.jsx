...
const heroVideo = "./Hero Talking Video.mp4";
...
const Hero = () => {
  ...
  const toggleVideo = (e) => {
    ...
    if (videoRef.current) {
      if (videoRef.current.paused || videoRef.current.ended) {
        // If video ended, rewind to beginning for replay
        if (videoRef.current.ended) {
          videoRef.current.currentTime = 0;
        }
        // Unmute and set full volume on user explicit click
  ...
  const handleVideoEnded = () => {
    setIsPlaying(false);
    // Keep video paused at end; do not auto-restart or loop
    if (videoRef.current) {
      videoRef.current.pause();
    }
  };

  const hasSocials = socialLinks.github || socialLinks.linkedin || socialLinks.leetcode;
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black flex items-center">
      ...
      {heroVideo && !videoError ? (
        ...
        <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(255,42,42,0.15),rgba(255,255,255,0))] bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
          <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-[#ff2a2a]/10 rounded-full blur-[140px] pointer-events-none" />
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-red-900/10 rounded-full blur-[160px] pointer-events-none" />
        </div>
      )}
      ...
      {/* Left Floating Social Bar for Large Screens (Conditional) */}
      {hasSocials && (
        <div className="hidden lg:flex flex-col gap-6 fixed left-6 top-1/2 -translate-y-1/2 z-50 mix-blend-difference">
          ...
          {socialLinks.leetcode && (
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-white/60 hover:text-[#FFA116] transition-all duration-300 transform hover:scale-125"
              aria-label="LeetCode"
            >
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M13.483 0a1.374 1.374 0 0 0-.961.438L7.116 6.226l-3.854 4.126a5.266 5.266 0 0 0-1.209 2.104 5.35 5.35 0 0 0-.125.513 5.527 5.527 0 0 0 .062 2.362 5.83 5.83 0 0 0 .349 1.017 5.938 5.938 0 0 0 1.271 1.818l4.277 4.193.039.038c2.248 2.165 5.852 2.133 8.063-.074l2.396-2.392c.54-.54.54-1.414.003-1.955a1.378 1.378 0 0 0-1.951-.003l-2.396 2.392a3.021 3.021 0 0 1-4.205.038l-.02-.019-4.276-4.193c-.652-.64-.972-1.469-.948-2.263a2.68 2.68 0 0 1 .666-1.607L9.82 9.074l4.624-4.947a1.374 1.374 0 0 0-.961-2.127z"/>
                <path d="M14.882 11.238a1.38 1.38 0 0 0-.974.404l-3.567 3.567a1.38 1.38 0 0 0 0 1.952 1.38 1.38 0 0 0 1.952 0l3.567-3.567a1.38 1.38 0 0 0-.978-2.356zm-1.897-4.258a1.38 1.38 0 0 0-.974.404l-7.135 7.135a1.38 1.38 0 0 0 0 1.952 1.38 1.38 0 0 0 1.952 0l7.135-7.135a1.38 1.38 0 0 0-.978-2.356z"/>
              </svg>
            </a>
          )}
        </div>
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          <p 
            data-aos="fade-up"
            data-aos-delay="200"
            className="text-white/90 text-sm md:text-base lg:text-lg font-semibold mb-8 max-w-2xl leading-relaxed drop-shadow-md"
          >
